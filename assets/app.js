...
const FEEDBACK_FORM_URL = "https://forms.gle/NxN7JojjcNXT2Msv6"; // TODO: Googleフォーム作成後にURLを差し替える
...
<p class="manual-warn">⚠ 登録したデータはこのブラウザ内（localStorage）にのみ保存され、システム側で永続的に管理されるものではありません。ブラウザのデータ消去や端末変更で失われるため、「データ保存（バックアップ）」から定期的にバックアップを保存してください。</p>
...
<dl>
  ...
  <dt>バックアップ統合／自動保存管理</dt><dd>複数JSONの統合、世代管理（重要な世代は固定可）</dd>
</dl>
</div>
</details>
...
<p class="manual-a">A. このブラウザのlocalStorageのみ。他端末への移行は「データ保存」→「データ読込」で。</p>
