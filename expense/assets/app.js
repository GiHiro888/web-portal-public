...
function getTaxParts(e) {
  ...
  if (e.taxRate === "10" || e.taxRate === "8")
    return [{ rate: e.taxRate, amount: e.amount }];
  return [];
}
...
window.onload = () => {
  document.getElementById("date").value = new Date()
    .toISOString()
    .split("T")[0];
  document.getElementById("print-date").value = new Date()
    .toISOString()
    .split("T")[0];
  setupEventListeners();
  loadFromStorage();
...
function updateShopSuggestions() {
  ...
  const combined = [
    ...new Set([...saved, ...history.filter((h) => !saved.includes(h))]),
  ];
  dl.innerHTML = combined
...
function setupEventListeners() {
  ...
  document
    .getElementById("settle-all-btn")
    .addEventListener("click", () => doSettlement(false));
  document
    .getElementById("settle-selected-btn")
    .addEventListener("click", () => doSettlement(true));
  document
    .getElementById("settle-name-toggle")
    .addEventListener("change", (e) => {
      document.getElementById("settle-name").style.display = e.target.checked
        ? ""
        : "none";
    });
  document
    .getElementById("print-date-manual")
    .addEventListener("change", (e) => {
      document.getElementById("print-date").style.display = e.target.checked
        ? ""
        : "none";
    });
  document
    .getElementById("print-empid-toggle")
    .addEventListener("change", (e) => {
      document.getElementById("print-empid").style.display = e.target.checked
        ? ""
        : "none";
    });
  document
    .getElementById("print-name-toggle")
    .addEventListener("change", (e) => {
      document.getElementById("print-name").style.display = e.target.checked
        ? ""
        : "none";
    });
  ...
  document
    ...
    .addEventListener("click", (e) => toggleAll(e.target));
  ...
  document
    .getElementById("merge-data-btn")
    .addEventListener("click", importMergeData);
  document
    .getElementById("backup-manage-btn")
    .addEventListener("click", showBackupManager);
  document
    .getElementById("review-files-btn")
    .addEventListener("click", openFileReview);

  document
    .getElementById("autosave-setup-btn")
    .addEventListener("click", setupAutoSaveFolder);
  document
    .getElementById("autosave-resume-btn")
    .addEventListener("click", resumeAutoSave);
  document
    .getElementById("autosave-disable-btn")
    .addEventListener("click", disableAutoSaveFolder);
  ...
  document.getElementById("modal-body").addEventListener("click", (e) => {
    ...
    if (revertBtn)
      revertToUnpaid(revertBtn.dataset.id, revertBtn.dataset.settlement);
    ...
    if (printSettlementBtn) {
      printTarget("settlement", {
        ...
        showAccounting: document.getElementById("print-show-accounting")
          .checked,
      });
    }

    // 精算内訳モーダル：Excel出力ボタン
    const excelSettlementBtn = e.target.closest(
      '.excel-export-btn[data-kind="settlement"]',
    );
  ...
  document
    .getElementById("unpaid-filter-clear")
    .addEventListener("click", () => {
      ["unpaid-filter-kw", "unpaid-filter-from", "unpaid-filter-to"].forEach(
        (id) => {
  ...
  document
    .getElementById("history-filter-clear")
    .addEventListener("click", () => {
      ["history-filter-kw", "history-filter-from", "history-filter-to"].forEach(
        (id) => {
          document.getElementById(id).value = "";
        },
      );
  ...
  document.getElementById("save-shop-btn").addEventListener("click", () => {
    ...
    if (!val) {
      alert("利用先を入力してから「★ 常用登録」してください。");
      return;
    }
    if (!db.savedShops) db.savedShops = [];
    if (db.savedShops.includes(val)) {
      alert("すでに常用店舗に登録されています。");
      return;
    }
  ...
  document
    .getElementById("saved-shops-chips")
    .addEventListener("click", (e) => {
      ...
      if (chip) {
        document.getElementById("shop").value =
          chip.dataset.shopIdx !== undefined
  ...
  document.querySelectorAll(".btn-save-kw").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const val = (document.getElementById(targetId).value || "")
        ...
        .slice(0, 50);
      if (!val) {
        alert("キーワードを入力してから「★ 登録」してください。");
        return;
      }
      if (!db.savedKeywords) db.savedKeywords = [];
      if (db.savedKeywords.includes(val)) {
        alert("すでに常用ワードに登録されています。");
        return;
      }
...
function positionHelpPopup(details) {
  ...
  top = Math.max(
    margin + ch / 2,
    Math.min(top, window.innerHeight - margin - ch / 2),
  );
...
function updateSplitTotal() {
  const total =
    splitFieldValue("split-10") +
    splitFieldValue("split-8") +
    splitFieldValue("split-none");
...
function setSplitMode(enabled) {
  ...
  document
    .getElementById("split-toggle-bar")
    .classList.toggle("active", enabled);
  document.getElementById("split-toggle-state").textContent = enabled
    ? "ON"
    : "OFF";
  document.getElementById("split-panel").style.display = enabled
    ? "flex"
    : "none";
...
function validateForm() {
  ...
  if (splitMode) {
    ...
    if (total <= 0) {
      setError(
        "amount",
        "税率分割欄（10%・8%・非課税のいずれか）に金額を入力してください",
      );
    }
  } else {
  ...
  if (invoice !== "" && /^[0-9]+$/.test(invoice) && invoice.length !== 13) {
    setError(
      "invoice",
      "インボイス登録番号は数字13桁（先頭のTは除く）で入力してください",
    );
  }
...
function applyPrintInfo() {
  ...
  const empid = empidOn
    ? document.getElementById("print-empid").value.trim()
    : "";
  ...
  const dateStr =
    manual && manualDate ? manualDate : new Date().toISOString().split("T")[0];

  document
    .querySelectorAll(".print-field-empid")
    .forEach((el) => (el.style.display = empidOn ? "" : "none"));
  document
    .querySelectorAll(".print-field-name")
    .forEach((el) => (el.style.display = nameOn ? "" : "none"));
  setPrintField(".print-empid-val", empid);
  setPrintField(".print-name-val", name);
  const dateJP = formatDateJP(dateStr);
  document
    .querySelectorAll(".print-doc-date")
    .forEach((el) => (el.textContent = dateJP));
}
...
function isValidExpense(e) {
  ...
  if (typeof e.shop !== "string" || e.shop.length > MAX_SHOP_LENGTH)
    return false;
  ...
  if (e.settlement_id !== undefined && e.settlement_id !== null) {
    if (
      typeof e.settlement_id !== "string" ||
      !ID_PATTERN.test(e.settlement_id)
    )
      return false;
  }
  if (e.memo !== undefined) {
    if (typeof e.memo !== "string" || e.memo.length > MAX_MEMO_LENGTH)
      return false;
  }
  if (e.payment !== undefined) {
    if (
      typeof e.payment !== "string" ||
      (e.payment !== "" && !PAYMENT_METHODS.includes(e.payment))
    )
      return false;
  }
  if (e.taxRate !== undefined) {
    if (
      typeof e.taxRate !== "string" ||
      (e.taxRate !== "" && !TAX_RATES.includes(e.taxRate))
    )
      return false;
  }
...
function isValidSettlement(s) {
  ...
  if (s.name !== undefined) {
    if (
      ...
      s.name.length > MAX_SETTLEMENT_NAME_LENGTH + 4
    )
      return false;
  }
  return true;
...
function isValidDb(obj) {
  ...
  if (!Array.isArray(obj.expenses) || !Array.isArray(obj.settlements))
    return false;
  return (
    obj.expenses.every(isValidExpense) &&
    obj.settlements.every(isValidSettlement)
  );
}
...
function mergeDb(base, incoming) {
  ...
  const existingSettlementIds = new Set(
    base.settlements.map((s) => String(s.id)),
  );
  ...
  const merged = {
    ...
    savedShops: [
      ...new Set([...(base.savedShops || []), ...(incoming.savedShops || [])]),
    ],
    savedKeywords: [
      ...new Set([
        ...(base.savedKeywords || []),
        ...(incoming.savedKeywords || []),
      ]),
    ],
  };
  ...
  const orphanExpenseIds = merged.expenses
    .filter(
      (e) =>
        ...
        e.settlement_id !== null &&
        !allSettlementIds.has(String(e.settlement_id)),
    )
    .map((e) => String(e.id));
...
function updateAutoSaveStatus() {
  ...
  if (!autoSaveSupported()) {
    text.textContent =
      "フォルダへの自動保存：このブラウザでは未対応です（Chrome・Edge等のブラウザでご利用いただけます）";
...
function startAutoSaveTimer() {
  ...
  autoSaveTimerId = setInterval(() => {
    if (
      shouldAutoSave({
        dirty: autoSaveDirty,
        lastSaveAt: autoSaveLastSavedAt,
        now: Date.now(),
...
async function performAutoSave() {
  ...
  autoSaveSaving = true;
  try {
    const fileHandle = await autoSaveDirHandle.getFileHandle(
      autoSaveFilename(new Date()),
      { create: true },
    );
...
async function pruneAutoSaveFolder() {
  ...
  for (const name of targets) {
    try {
      await autoSaveDirHandle.removeEntry(name);
    } catch (_) {
      /* 個別の削除失敗は無視し継続 */
    }
  }
...
function showFolderGuideModal() {
  return new Promise((resolve) => {
    ...
    const checkCreated = document.getElementById(
      "autosave-guide-check-created",
    );
    const checkConfirmed = document.getElementById(
      "autosave-guide-check-confirmed",
    );
    ...
    function updateProceedState() {
      proceedBtn.disabled = !canProceedFolderGuide(
        checkCreated.checked,
        checkConfirmed.checked,
      );
    }
    function cleanup() {
...
async function setupAutoSaveFolder() {
  ...
  try {
    ...
    alert(
      `「${parent.name}」内に「${handle.name}」を用意しました。エクスプローラーで「${parent.name}」を開くと確認できます。`,
    );
  } catch (e) {
...
function disableAutoSaveFolder() {
  if (
    !confirm(
      "フォルダへの自動保存を解除します。これまで保存されたファイル自体は削除されません。よろしいですか？",
    )
  ) {
    returnFocus();
    return;
...
function addEntry() {
  ...
  if (editingId) {
    ...
    if (!expense || expense.status !== "未") {
      alert(
        "編集対象のデータが見つかりませんでした（他の操作で変更された可能性があります）。編集モードを解除します。",
      );
      cancelEdit();
      return;
    }
...
function startEdit(id) {
  ...
  if (Array.isArray(expense.taxParts) && expense.taxParts.length > 0) {
    ...
    document.getElementById("split-10").value = t10
      ? t10.amount.toLocaleString("en-US")
      : "";
    document.getElementById("split-8").value = t8
      ? t8.amount.toLocaleString("en-US")
      : "";
    const noneAmount = expense.amount - partsSum;
    document.getElementById("split-none").value =
      noneAmount > 0 ? noneAmount.toLocaleString("en-US") : "";
    setSplitMode(true);
  } else {
    document.getElementById("amount").value =
  ...
  document.getElementById("edit-mode-target").textContent =
    `${expense.date} ${expense.shop}（${formatYen(expense.amount)}）`;
...
function doSettlement(onlySelected = false) {
  ...
  if (onlySelected) {
    ...
    targetExpenses = db.expenses.filter(
      (e) => e.status === "未" && selectedIds.includes(String(e.id)),
    );
  } else {
  ...
  if (document.getElementById("settle-name-toggle").checked) {
    ...
    if (base) {
      if (base.length > MAX_SETTLEMENT_NAME_LENGTH) {
        alert(
          `精算名は${MAX_SETTLEMENT_NAME_LENGTH}文字以内で入力してください。`,
        );
        returnFocus();
        return;
      }
  ...
  if (
    !confirm(
      `${targetExpenses.length}件、合計 ${formatYen(total)} を${nameLabel}精算しますか？`,
    )
  ) {
    returnFocus();
    return;
...
function deleteSelected() {
  ...
  if (
    !confirm(
      `${selectedIds.length}件のデータを完全に消去します。この操作は取り消せません。\n本当に消去しますか？`,
    )
  ) {
    returnFocus();
    return;
...
function updateSelectionBar() {
  ...
  if (selected.length > 0) {
    ...
    const sum = selected.reduce(
      (s, cb) => s + parseFloat(cb.getAttribute("data-amount")),
      0,
    );
...
function getUnpaidFilter() {
  return {
    kw: (document.getElementById("unpaid-filter-kw").value || "")
      .trim()
      .toLowerCase(),
...
function getHistoryFilter() {
  return {
    kw: (document.getElementById("history-filter-kw").value || "")
      .trim()
      .toLowerCase(),
...
function matchUnpaidFilter(item) {
  ...
  if (f.kw) {
    const targets = [item.shop, item.memo, ...(item.items || [])].map((x) =>
...
function render() {
  ...
  const unpaidAll = db.expenses
    .filter((e) => e.status === "未")
    .sort((a, b) => b.date.localeCompare(a.date));
  const unpaidData = isUnpaidFilterActive()
    ? unpaidAll.filter(matchUnpaidFilter)
    : unpaidAll;
  ...
  document.getElementById("unpaid-count-display").innerText = unpaidFiltered
    ...
    : unpaidData.length;
  document.getElementById("unpaid-sum-display").innerText =
  ...
  document.getElementById("unpaid-filter-count").style.display = unpaidFiltered
    ? "block"
    : "none";
  document.getElementById("unpaid-filter-count").innerText =
    `全${unpaidAll.length}件中 ${unpaidData.length}件 を表示`;
  document.getElementById("unpaid-settle-note").style.display = unpaidFiltered
    ? "block"
    : "none";
  ...
  unpaidData.forEach((e) => {
    ...
    getTaxParts(e).forEach((p) => {
      ...
      if (p.rate === "10") {
        tax10Sum += p.amount;
        tax10TaxSum += calcTax(p.amount, "10").tax;
      } else if (p.rate === "8") {
        tax8Sum += p.amount;
        tax8TaxSum += calcTax(p.amount, "8").tax;
      }
    });
  ...
  document.getElementById("tax-none-sum-display").innerText =
    formatYen(taxNoneSum);
  document.getElementById("tax10-tax-display").innerText =
    formatYen(tax10TaxSum);
  document.getElementById("tax8-tax-display").innerText = formatYen(tax8TaxSum);

  unpaidBody.innerHTML = unpaidData.length
    ? ""
    : '<tr><td colspan="8" class="empty-msg">未精算のデータはありません</td></tr>';
  ...
  document.getElementById("history-count-display").innerText =
    db.settlements.length;
  document.getElementById("settled-item-count-display").innerText =
    settledItems.length;
  document.getElementById("history-limit-note").style.display =
    !historyFiltered && db.settlements.length > 10 ? "block" : "none";
  document.getElementById("history-filter-count").style.display =
    historyFiltered ? "block" : "none";
  document.getElementById("history-filter-count").innerText =
    `全${db.settlements.length}件中 ${historyData.length}件 を表示`;

  historyBody.innerHTML = historyData.length
    ? ""
    : '<tr><td colspan="3" class="empty-msg">履歴はありません</td></tr>';
  ...
  document.getElementById("orphan-card").style.display = showOrphan
    ? "block"
    : "none";
...
function revertOrphanToUnpaid(id) {
  ...
  if (!expense) return;
  if (
    !confirm(`この明細（${formatYen(expense.amount)}）を未精算に戻しますか？`)
  )
    return;
...
function deleteOrphan(id) {
  ...
  if (
    !confirm(
      `この明細（${formatYen(expense.amount)}）を完全に削除します。この操作は取り消せません。\n本当に削除しますか？`,
    )
  )
    return;
...
function showDetail(sId) {
  ...
  details.forEach((d) => {
    ...
    if (
      parts.length > 1 ||
      (Array.isArray(d.taxParts) && d.taxParts.length > 0)
    ) {
      taxText = parts
        .map((p) => {
          const tax = calcTax(p.amount, p.rate);
  ...
  document.getElementById("modal-title").innerHTML =
    `精算内訳 (<span style="white-space:nowrap;">${escapeHtml(settlement.name || sId)}</span>)<br><span style="font-size: 16px; color: #999; font-weight: normal;">（表内の金額の単位：円）</span>`;
...
function revertToUnpaid(id, sId) {
  ...
  if (remaining) {
    ...
    closeModal();
    alert(
      "未精算に戻しました。この精算の明細はすべて未精算に戻ったため、精算履歴を削除しました。",
    );
  }
}
...
function buildPrintTables() {
  const unpaidData = db.expenses
    .filter((e) => e.status === "未")
    .sort(getSortComparator("date", "shop"));
  document.getElementById("unpaid-print-count").textContent = unpaidData.length;
  let unpaidTotal = 0;
  document.getElementById("unpaid-print-list").innerHTML = unpaidData.length
    ? unpaidData
        .map((item) => {
        ...
        .join("")
    : '<tr><td colspan="6">未精算のデータはありません</td></tr>';
  document.getElementById("unpaid-print-total").textContent =
    unpaidTotal.toLocaleString();

  // 精算履歴：利用先名の列を持たないため「利用先名」並び順は精算名（s.name）で代替
  const historyData = [...db.settlements].sort(
    getSortComparator("date", "name"),
  );
  document.getElementById("history-print-count").textContent =
    historyData.length;
  document.getElementById("history-print-list").innerHTML = historyData.length
    ? historyData
        .map((s) => {
          const count = db.expenses.filter(
            (e) => e.settlement_id === s.id,
          ).length;
          return `<tr>
...
function paginatePrintDocument(docEl, tableSelector) {
  ...
  const preHtml = allChildren
    .slice(0, tableIndex)
    .map((el) => el.outerHTML)
    .join("");
  const postExtraHtml = allChildren
    .slice(tableIndex + 1)
    .map((el) => el.outerHTML)
    .join("");
  ...
  const preHeaderHeight = measure.querySelector(
    '[data-measure="pre"]',
  ).offsetHeight;
  const postExtraHeight = measure.querySelector(
    '[data-measure="post"]',
  ).offsetHeight;
  const footerHeight = measure.querySelector(
    '[data-measure="footer"]',
  ).offsetHeight;
  ...
  const rowHeights = rowEls.map(
    (_, i) => measure.querySelector(`tr[data-pgidx="${i}"]`).offsetHeight,
  );
  ...
  const regularBudget =
    PAGE_CONTENT_HEIGHT_PX - preHeaderHeight - theadHeight - footerHeight;
  ...
  const lastPageHeight = pages[pages.length - 1].reduce(
    (sum, i) => sum + rowHeights[i],
    0,
  );
...
function printTarget(mode, options = {}) {
  ...
  let docEl = null,
    tableSelector = "";
  if (mode === "unpaid") {
    docEl = document.querySelector("#unpaid-card > .print-document");
    tableSelector = ".print-table";
  } else if (mode === "history") {
    docEl = document.querySelector("#history-card > .print-document");
    tableSelector = ".print-table";
  } else if (mode === "settlement") {
    docEl = document.querySelector("#modal .print-document");
    tableSelector = ".print-table.settlement";
  }
...
function openFeedbackForm() {
  if (
    !FEEDBACK_FORM_URL ||
    FEEDBACK_FORM_URL.includes("https://forms.gle/1PJsVJUe1WxG6mNg8")
  ) {
    alert("ご意見フォームは準備中です。");
    returnFocus();
    return;
  ...
  input.onchange = (e) => {
    ...
    Promise.all(readers).then((results) => {
      ...
      if (validResults.length === 0) {
        alert(
          "有効なバックアップファイルが見つかりませんでした（JSON解析エラーまたは構造不正）。",
        );
        returnFocus();
        return;
      }
...
function renderFileReview() {
  const settlementIds = new Set(
    fileReviewState.settlements.map((s) => String(s.id)),
  );
  const settlementsById = new Map(
    fileReviewState.settlements.map((s) => [String(s.id), s]),
  );
  const expenses = [...fileReviewState.expenses].sort((a, b) =>
  ...
  expenses.forEach((item) => {
    ...
    const isOrphan =
      item.settlement_id !== undefined &&
      item.settlement_id !== null &&
      !settlementIds.has(String(item.settlement_id));
    ...
    if (frEditingId === id) {
      const paymentOptions = ['<option value="">（未選択）</option>']
        .concat(
          PAYMENT_METHODS.map(
            (p) =>
              `<option value="${escapeHtml(p)}" ${item.payment === p ? "selected" : ""}>${escapeHtml(p)}</option>`,
          ),
        )
        .join("");
      const taxRateOptions = ['<option value="">（未分類）</option>']
        .concat(
          TAX_RATES.map(
            (r) =>
              `<option value="${escapeHtml(r)}" ${item.taxRate === r ? "selected" : ""}>${escapeHtml(r)}%</option>`,
          ),
        )
        .join("");
      ...
      html += `</div>`;
      if (hasParts) {
        const partsText = item.taxParts
          .map((p) => `${escapeHtml(p.rate)}%：${p.amount.toLocaleString()}円`)
          .join(" / ");
      ...
      if (
        parts.length > 1 ||
        (Array.isArray(item.taxParts) && item.taxParts.length > 0)
      ) {
        taxText = parts
          .map((p) => `${p.rate}%対象${p.amount.toLocaleString()}`)
          .join(" / ");
      ...
      if (taxText) html += `<span>${taxText}</span>`;
      const isSettled =
        item.settlement_id !== undefined && item.settlement_id !== null;
      if (isSettled) {
      ...
      html += `<div class="detail-row-action">`;
      if (!isSettled)
        html += `<button class="btn-sub fr-edit-btn" data-id="${escapeHtml(id)}">編集</button> `;
  ...
  if (fileReviewState.settlements.length === 0) {
    ...
    html +=
      '<table><thead><tr><th>精算日</th><th>精算ID</th><th style="width:150px;">合計金額</th><th style="width:100px;">操作</th></tr></thead><tbody>';
    [...fileReviewState.settlements]
      .sort((a, b) => b.date.localeCompare(a.date))
      .forEach((s) => {
...
function frShowSettlementDetail(sId) {
  const settlement = fileReviewState.settlements.find(
    (s) => String(s.id) === String(sId),
  );
  const details = fileReviewState.expenses.filter(
    (e) => String(e.settlement_id) === String(sId),
  );
...
function frSaveEdit(id) {
  ...
  const hasParts =
    Array.isArray(expense.taxParts) && expense.taxParts.length > 0;
  const taxRate = hasParts
    ? expense.taxRate
    : document.getElementById(`fr-edit-taxrate-${id}`).value;
  const amount = hasParts
    ? expense.amount
    : Number(
        document
          .getElementById(`fr-edit-amount-${id}`)
          .value.replace(/[^\d]/g, ""),
      );
  ...
  if (
    ...
    alert(
      `金額は1以上${MAX_AMOUNT.toLocaleString()}以下の整数で入力してください。`,
    );
    return;
  }
  ...
  if (!isValidExpense(candidate)) {
    alert(
      "入力内容が正しくありません。日付・利用先・金額などを確認してください。",
    );
    return;
  }
...
function frDeleteExpense(id) {
  if (
    !confirm(
      "この明細を整理対象から消去します。よろしいですか？\n（書き出し・統合読込の対象から除外されます。元のファイルは変更されません）",
    )
  )
    return;
  fileReviewState.expenses = fileReviewState.expenses.filter(
    (e) => String(e.id) !== id,
  );
...
function frDeleteSettlement(id) {
  if (
    !confirm(
      "この精算履歴を整理対象から消去します。よろしいですか？\n（参照する経費明細は「孤児」として表示されます）",
    )
  )
    return;
  fileReviewState.settlements = fileReviewState.settlements.filter(
    (s) => String(s.id) !== id,
  );
  renderFileReview();
}
...
function frExportConsolidated() {
  const blob = new Blob([JSON.stringify(fileReviewState, null, 2)], {
    type: "application/json",
  });
  downloadBlob(
    blob,
    `expense_consolidated_${new Date().toISOString().split("T")[0]}.json`,
  );
}
...
function frMergeIntoCurrent() {
  ...
  if (result.skippedExpenseIds.length > 0)
    msg += `重複のためスキップされる経費明細：${result.skippedExpenseIds.length} 件\n`;
  if (result.skippedSettlementIds.length > 0)
    msg += `重複のためスキップされる精算履歴：${result.skippedSettlementIds.length} 件\n`;
...
function showBackupManager() {
  const pinnedKeys = Object.keys(localStorage)
    .filter((k) => k.startsWith(PINNED_PREFIX))
    .sort()
    .reverse();
  const normalKeys = Object.keys(localStorage)
    .filter(
      (k) => k.startsWith(BACKUP_PREFIX) || k.startsWith(CORRUPTED_PREFIX),
    )
    .sort()
    .reverse();
  ...
  if (totalCount === 0) {
    ...
    html +=
      '<table><thead><tr><th>種類</th><th>作成日時</th><th style="width:260px;">操作</th></tr></thead><tbody>';
    // 固定バックアップを先に表示
    ...
    normalKeys.forEach((key) => {
      ...
      const ts = Number(
        key.slice(isCorrupted ? CORRUPTED_PREFIX.length : BACKUP_PREFIX.length),
      );
      const dateStr = Number.isFinite(ts)
        ? new Date(ts).toLocaleString("ja-JP")
        : "-";
...
function pinBackup(key) {
  const pinnedCount = Object.keys(localStorage).filter((k) =>
    k.startsWith(PINNED_PREFIX),
  ).length;
  if (pinnedCount >= MAX_PINNED) {
    alert(
      `固定できるバックアップは最大${MAX_PINNED}件です。\n既存の固定を解除してから固定してください。`,
    );
    return;
  }
  const data = localStorage.getItem(key);
  ...
  const ts = key.startsWith(BACKUP_PREFIX)
    ? key.slice(BACKUP_PREFIX.length)
    : key.slice(CORRUPTED_PREFIX.length);
...
function exportCSV() {
  ...
  let csvContent =
    "\ufeff日付,利用先,金額,10%対象額,8%対象額,摘要,支払方法,税率,精算状態,精算ID\n";
  [...db.expenses]
    .sort((a, b) => b.date.localeCompare(a.date))
    .forEach((item) => {
      let tax10Amount = 0,
      ...
      const row = [
        ...
        csvSafe(item.status),
        csvSafe(
          (settlementsById.get(String(item.settlement_id)) || {}).name ||
            item.settlement_id ||
            "",
        ),
      ];
...
function importCSV() {
  ...
  input.onchange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      if (addList.length === 0) {
        alert(
          "取込可能な明細がありませんでした（形式が正しいCSVファイルかご確認ください）。読み込みを中止しました。",
        );
        returnFocus();
        return;
      }
      ...
      render();
      alert(
        `${addList.length}件の明細を追加しました。${invalidCount > 0 ? `（${invalidCount}件スキップ）` : ""}`,
      );
      returnFocus();
    };
    reader.readAsText(file);
...
function xlsCrc32(bytes) {
  ...
  for (let i = 0; i < bytes.length; i++)
    crc = (crc >>> 8) ^ xlsCrc32.table[(crc ^ bytes[i]) & 0xff];
  return (crc ^ -1) >>> 0;
}
...
function readZipEntries(buf) {
  ...
  for (let i = 0; i < entryCount; i++) {
    if (dv.getUint32(cdOffset, true) !== 0x02014b50)
    ...
    const name = new TextDecoder().decode(
      u8.subarray(cdOffset + 46, cdOffset + 46 + nameLen),
    );
...
async function injectPageSetup(zipArrayBuffer, scale, centered) {
  ...
  const xmlBytes =
    sheetEntry.method === 8
      ? await inflateRawZip(sheetEntry.data)
      : sheetEntry.data;
  ...
  const printOptionsTag = centered
    ? '<printOptions horizontalCentered="1"/>'
    : "";
  xml = xml.replace(
    /<pageMargins[^/]*\/>/,
    (m) => `${printOptionsTag}${m}${pageSetupTag}`,
  );
...
function buildExcelAoaUnpaid() {
  ...
  aoa.push([
    `申請日：${info.dateJP}　対象件数：${data.length}件`,
    "",
    "",
    "",
    "",
  ...
  return {
    ...
    cols: colWidths,
    margins,
    pageSetup: { scale: 100, centered: false },
  };
}
...
function buildExcelAoaHistory() {
  ...
  return {
    ...
    cols: colWidths,
    margins,
    pageSetup: { scale: 135, centered: true },
  };
}
...
function buildExcelAoaSettlement(sId) {
  ...
  const showAccounting = document.getElementById(
    "print-show-accounting",
  ).checked;
  const cols = 8;
  const aoa = [];
  ...
  [0, 3, 6].forEach((c) =>
    styles.push({ r: metaRow, c, style: xlsStyle.header }),
  );
  ...
  sorted.forEach((d, i) => {
    ...
    aoa.push(rowVals);
    widthRows.push(rowVals);
    [0, 1, 2, 4, 5, 6, 7].forEach((c) =>
      styles.push({ r, c, style: xlsStyle.body }),
    );
    styles.push({ r, c: 3, style: xlsStyle.bodyNum });
  });
  ...
  if (approvalLabels.length) {
    ...
    approvalLabels.forEach((label, i) => {
      ...
      if (c1 > c0)
        merges.push({
          s: { r: approvalRow, c: c0 },
          e: { r: approvalRow, c: c1 },
        });
  ...
  return {
    ...
    cols: colWidths,
    margins,
    pageSetup: { scale: 75, centered: false },
  };
}
...
async function downloadExcelAoa(doc, filename, sheetName) {
  ...
  if (doc.pageSetup) {
    out = await injectPageSetup(
      out,
      doc.pageSetup.scale,
      doc.pageSetup.centered,
    );
  }
  downloadBlob(
    new Blob([out], {
...
function exportExcel(kind, sId) {
  ...
  if (kind === "unpaid") {
    if (db.expenses.filter((e) => e.status === "未").length === 0) {
      alert("出力する未精算データがありません。");
      return;
    }
  ...
  ensureXLSX()
    .then(() => downloadExcelAoa(doc, filename, sheetName))
    .then(() => showExcelTipOnce())
    .catch(() =>
      alert(
        "Excel機能の読込に失敗しました。通信環境をご確認のうえ再度お試しください。",
      ),
    );
}
  ...
  input.onchange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        alert(
          "ファイルの形式が正しくありません（JSON解析エラー）。読み込みを中止しました。",
        );
        returnFocus();
        return;
      }
...
function clearAll() {
  ...
  if (
    confirm(
      "消去前に現在のデータをバックアップ（JSONファイル）として保存しますか？",
    )
  ) {
    exportData();
  }
  if (
    !confirm(
      "最終確認：すべてのデータを消去します。この操作は取り消せません。\n本当に実行しますか？",
    )
  ) {
    returnFocus();
    return;
